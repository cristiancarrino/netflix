...
$favorite_genres = json_decode($jsonPostdata);

$sql = "UPDATE user SET
        favorite_genres = " . ($favorite_genres->ids ? "'" . $favorite_genres->ids . "'" : "NULL") . "
        WHERE token = '" . (isset($headers['Authorization']) ? $headers['Authorization'] : $headers['authorization']) . "'";
...
echo '{ "success": true, "message": "Favorite genres edited successfully" }';
